...
│  │                                                  │    │
│  │  • Watches timeline/data/events/*.json           │    │
...
priority=$(curl -s http://localhost:5558/api/priorities/next)
...
curl -X POST http://localhost:5558/api/events/validate \
  -d @event.json
...
existing=$(curl -s "http://localhost:5558/api/events/search?q=halliburton+no-bid&date=2003-03-24")
...
curl http://localhost:5558/api/stats | jq '.priorities.pending'

# Log activity for tracking
curl -X POST http://localhost:5558/api/activity \
